...
std::vector<Eigen::Vector2f> ORBSLAM3Python::getCurrentKeyPoints() const
{
    ...
    try {
        // Получаем текущий кадр трекера
        ORB_SLAM3::Tracking* tracker = system->GetTracker();
        if (!tracker) return keyPoints;

        // Получаем ключевые точки из текущего кадра
        std::vector<cv::KeyPoint> vCurrentKeys = tracker->GetCurrentFrame().mvKeys;
        keyPoints.reserve(vCurrentKeys.size());

        for (const cv::KeyPoint& kp : vCurrentKeys)
        {
            keyPoints.push_back(Eigen::Vector2f(kp.pt.x, kp.pt.y));
        }
    } catch (...) {
        // Если что-то пошло не так, возвращаем пустой вектор
    }
